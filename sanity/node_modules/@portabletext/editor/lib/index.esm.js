...
import { useSlateStatic, ReactEditor, useSelected, useSlate, withReact, Slate, Editable } from "slate-react";
...
function useSyncValue(props) {
  const {
    ...
    portableTextEditor,
    readOnly
  } = props, schemaTypes = editorActor.getSnapshot().context.schema, previousValue = useRef(), slateEditor = useSlate(), updateValueFunctionRef = useRef(), updateFromCurrentValue = useCallback(() => {
...
function insertPatch(editor, patch, schemaTypes) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$g(`Inserting blocks at path [${normalizedIdx2}]`), debugState(editor, "before"), Transforms.insertNodes(editor, blocksToInsert, {
      at: [normalizedIdx2]
  ...
  return debug$g(`Inserting children at path ${childInsertPath}`), debugState(editor, "before"), childrenToInsert && Element$1.isElement(childrenToInsert[0]) && Transforms.insertNodes(editor, childrenToInsert[0].children, {
    at: childInsertPath
  }), debugState(editor, "after"), !0;
}
function setPatch(editor, patch) {
  ...
  if (debugState(editor, "before"), isTextBlock && child && childPath) {
    ...
    }), debug$g("Setting children"), block.children.forEach((c2, cIndex) => {
      editor.apply({
        type: "remove_node",
...
function unsetPatch(editor, patch) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$g(`Removing block at path [${blockIndex}]`), debugState(editor, "before"), Transforms.removeNodes(editor, {
      at: [blockIndex]
    }), debugState(editor, "after"), !0;
  }
  return editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3 ? !child || !childPath ? (debug$g("Child not found"), !1) : (debug$g(`Unsetting child at path ${JSON.stringify(childPath)}`), debugState(editor, "before"), debugVerbose$2 && debug$g(`Removing child at path ${JSON.stringify(childPath)}`), Transforms.removeNodes(editor, {
    at: childPath
  ...
  return function(editor) {
    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      editor.selection && ([...Editor.nodes(editor, {
        ...
        editor.isTextBlock(node) && node.style === blockStyle ? (debug$d(`Unsetting block style '${blockStyle}'`), Transforms.setNodes(editor, {
          ...node,
          style: defaultStyle
        }, {
          at: path
        })) : (blockStyle ? debug$d(`Setting style '${blockStyle}'`) : debug$d("Setting default style", defaultStyle), Transforms.setNodes(editor, {
          ...node,
    ...
    return editor.pteToggleListItem = (listItemStyle) => {
      ...
      editor.selection && [...Editor.nodes(editor, {
        ...
        if (editor.isListBlock(node)) {
          ...
          debug$c(`Unsetting list '${listItemStyle}'`), Transforms.setNodes(editor, newNode, {
            at: path
          });
        }
      });
      ...
      editor.selection && [...Editor.nodes(editor, {
        ...
        debug$c(`Setting list '${listItemStyle}'`), Transforms.setNodes(editor, {
          ...node,
          level: 1,
      ...
      return selectedBlocks.length === 0 ? !1 : (selectedBlocks.forEach(([node, path]) => {
        Element$1.isElement(node) && (debug$c("Unset list"), Transforms.setNodes(editor, {
          ...node,
          level: void 0,
          listItem: void 0
        }, {
      ...
      return selectedBlocks.length === 0 ? !1 : (selectedBlocks.forEach(([node, path]) => {
        if (editor.isListBlock(node)) {
          ...
          reverse ? (level--, debug$c("Decrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)))) : (level++, debug$c("Incrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)))), Transforms.setNodes(editor, {
...
const EditorActorContext = createContext({}), debug$6 = debugWithName("API:editable");
...
function useEditor(config) {
  ...
  let t2;
  $[3] !== config.behaviors || $[4] !== t0 || $[5] !== t1 ? (t2 = {
    input: {
